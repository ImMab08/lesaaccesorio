...
export default function DeleteArticulo({ articulo, setData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await eliminar(`/v01/articulo/delete/${articulo?.id}`)
    setData((prev) => 
      prev.filter((item) => item.id !== articulo.id)
    );
    ...
    <div className="w-full h-full top-0 left-0 bg-black/70 bg-opacity-60 fixed z-50 flex items-center justify-center">
      <div className="rounded-lg bg-secondary w-[450px] ">
        <div className="flex flex-col space-y-4 p-5">
          <p className="text-xl text-center">¿Estás seguro que deseas eliminar la articulo <span className="font-semibold">{articulo?.nombre}</span>?</p>
        </div>
