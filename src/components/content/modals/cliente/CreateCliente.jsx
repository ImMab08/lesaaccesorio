...
export default function CreateCliente({ clientes, setData }) {
  const { closeModal } = useModalStore();

  // Estados del formulario.
  const [ nombre, setNombre ] = useState("");
  const [ apellido, setApellido ] = useState("");
  const [ cliente_email, setEmail ] = useState("");
  const [ cliente_direccion, setDireccion ] = useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await crear("/v01/cliente/create", { nombre, apellido, cliente_email, cliente_direccion, telefono });
    setData([
      ...clientes,
      response,      
    ])
    ...
    <div className="text-title w-full h-full top-0 left-0 bg-black/70 bg-opacity-60 fixed z-50 flex items-center justify-center">
      <div className="rounded-lg bg-secondary">
        <div className="flex flex-col space-y-1.5 p-5">
          <h3 className="text-title text-xl font-semibold text-center">Añadir un nuevo cliente</h3>
        </div>
